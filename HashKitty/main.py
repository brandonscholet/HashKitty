...
"""    
def group_lines_by_hash(data):
    hash_dict = defaultdict(list)
    # Adjust regex pattern to specifically capture the NT hash portion
    hash_pattern = re.compile(r':[^:]*:[^:]*:(?P<password_hash>[0-9a-fA-F]+):')

    for line in data:
        if "31d6cfe0d16ae931b73c59d7e0c089c0" not in line:
            match = hash_pattern.search(line)
            if match:
                password_hash = match.group('password_hash')
                hash_dict[password_hash].append(line)
    
    # Filter out unique instances
    filtered_hash_dict = {k: v for k, v in hash_dict.items() if len(v) > 1}
    grouped_lines = sorted(filtered_hash_dict.values(), key=len, reverse=True)
